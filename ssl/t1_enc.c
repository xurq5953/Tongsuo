...
int tls1_change_cipher_state(SSL_CONNECTION *s, int which)
{
    ...
    if (!ssl_set_new_record_layer(s, s->version, direction,
                                    ...
                                    mac_secret_size, c, taglen, EVP_PKEY_HMAC,
                                    m, comp, NULL)) {
...
int tls1_setup_key_block(SSL_CONNECTION *s)
{
    ...
    if (SSL_CONNECTION_IS_NTLS(s) && (s->options & SSL_OP_NO_TICKET)
        && (SSL_CONNECTION_GET_CTX(s)->session_cache_mode == SSL_SESS_CACHE_OFF)) {
