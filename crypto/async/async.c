                    ...
                    int (*func)(void *), void *args, size_t size)
{
    ...
    for (;;) {
        ...
        /* Start a new job */
        if ((new = async_get_pool_job()) == NULL)
            return ASYNC_NO_JOBS;

        if (args != NULL) {
            new->funcargs = OPENSSL_malloc(size);
            if (new->funcargs == NULL) {
                async_release_job(new);
                return ASYNC_ERR;
            }
            memcpy(new->funcargs, args, size);
        } else {
            new->funcargs = NULL;
        }
