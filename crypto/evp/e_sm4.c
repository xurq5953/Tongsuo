...
static const EVP_CIPHER sm4_##mode = { \
        nid##_##mode,blocksize, 128/8, ivlen, \
        flags|EVP_CIPH_##MODE##_MODE,   \
        EVP_ORIG_GLOBAL,                \
        sm4_##mode##_init,              \
        sm4_##mode##_cipher,            \
        ...
        NULL,NULL,sm4_##mode##_ctrl,NULL }; \
const EVP_CIPHER *EVP_sm4_##mode(void) \
{ return &sm4_##mode; }
...
# ifdef HWSM4_CAPABLE
            if (HWSM4_CAPABLE) {
                HWSM4_set_encrypt_key(key, &gctx->ks);
                CRYPTO_gcm128_init(&gctx->gcm, &gctx->ks,
                                   (block128_f)HWSM4_encrypt);
#  ifdef HWSM4_ctr32_encrypt_blocks
                gctx->ctr = (ctr128_f)HWSM4_ctr32_encrypt_blocks;
#  else /* HWSM4_ctr32_encrypt_blocks */
                gctx->ctr = (ctr128_f)NULL;
#  endif
                break;
            } else
# endif /* HWSM4_CAPABLE */
...
# ifdef HWSM4_CAPABLE
            if (HWSM4_CAPABLE) {
                HWSM4_set_encrypt_key(key, &cctx->ks);
                CRYPTO_ccm128_init(&cctx->ccm, cctx->M, cctx->L,
                                   &cctx->ks, (block128_f)HWSM4_encrypt);
                cctx->str = NULL;
                cctx->key_set = 1;
                break;
            } else
...
BLOCK_CIPHER_custom(NID_sm4, 1, 12, gcm, GCM,
                    EVP_CIPH_FLAG_AEAD_CIPHER | CUSTOM_FLAGS)
BLOCK_CIPHER_custom(NID_sm4, 1, 12, ccm, CCM,
                    EVP_CIPH_FLAG_AEAD_CIPHER | CUSTOM_FLAGS)
#endif
